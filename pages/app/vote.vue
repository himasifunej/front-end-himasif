<template>
  <div class="bg-primary-950 text-white min-h-screen relative">
    ...
    <AppContentArea class="pt-4 relative z-10">
      <!-- ========== HEADER BARU (TEXT KIRI + GAMBAR KANAN) ========== -->
      <AppContainer class="w-2/3 pb-10 text-white">
        <div class="flex flex-col lg:flex-row justify-between items-center">
          <!-- TEXT KIRI -->
          <div class="text-center lg:text-left max-w-xl space-y-2">
            <h1 class="text-title-2 font-bold">PEMILIHAN KETUA UMUM</h1>
            <h2 class="text-title-3 font-semibold">HIMPUNAN MAHASISWA SISTEM INFORMASI</h2>
            <h2 class="text-title-3 font-semibold">UNIVERSITAS JEMBER</h2>
            <h3 class="text-title-4 font-medium">PERIODE 2025 / 2026</h3>
          </div>

          <!-- GAMBAR KANAN -->
          <div class="mt-6 lg:mt-0 lg:ml-10">
            <NuxtImg
              src="/vote-icon.png"
        ...
        <div class="flex justify-center">
          <template>
            ...
            <div
              ...
              tabindex="-1"
              aria-hidden="true"
              class="hidden overflow-y-auto fixed top-0 right-0 left-0 z-50 justify-center items-center w-full max-h-full bg-black/80"
            >
...
const isVoteTime = computed(() => now.value > new Date(vote_settings.value.result.start_at).getTime());
...
const submitVote = async () => {
  if (now.value > new Date(vote_settings.value.result.end_at)) {
    alertStore.openModal({ component: markRaw(AppAlertError), props: { text: "Halo Sisforians, Maaf vote telah ditutup" } });
    return;
  }

  await alertStore
    .openModal({ component: markRaw(AppAlertConfirm), props: { text: "Apakah kamu yakin dengan pilihanmu?" } })
    .then(async (res) => {
      if (res) {
