                  ...
                  <div class="p-8 flex">
                    ...
                    <div class="w-3/4 space-y-6 flex flex-col justify-center">
                      <div class="space-y-4">
                        ...
                        <div class="text-secondary" v-html="infoKetua?.visi" />
                      </div>
...
const ERROR_PLACEHOLDER = "https://placehold.co/600x400?text=Image+Error";

const getImageUrl = (ketua) => {
  if (!ketua || !ketua.image || !Array.isArray(ketua.image)) {
    return PLACEHOLDER;
  }
  if (ketua.image.length === 0) {
    return PLACEHOLDER;
  }
  if (!ketua.image[0].url) {
    return PLACEHOLDER;
  }
  return ketua.image[0].url;
};

const handleImageError = (event) => {
  event.target.src = ERROR_PLACEHOLDER;
};
